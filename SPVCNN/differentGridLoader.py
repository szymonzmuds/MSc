...
class DiffrentSemanticKITTI(dict):
    def __init__(self, root, voxel_size, num_points, **kwargs):
        ...
        if submit_to_server:
            super(DiffrentSemanticKITTI, self).__init__({
                'train':
                DmodifyPointCloudSemanticKITTIInternal(root,
                                      voxel_size,
                                      num_points,
                                      sample_stride=1,
                                      split='train',
...
class DmodifyPointCloudSemanticKITTIInternal:
    def __init__(self,
                 root,
                 voxel_size,
                 num_points,
...
#       block[np.absolute(block[:, 1]) > mod] = np.sign(block[np.absolute(block[:, 1]) > mod]) * (mod + (np.absolute(block[np.absolute(block[:, 1]) > mod]) -mod) / divider)
        ...
        pc_[pc_ >= lim2] = 400 + np.round((pc_[pc_>=lim2]) / gridSize1)
        # pc_[np.logical_and(pc_ >= lim1, pc_ < lim2)] = 300 +  np.round((pc_[np.logical_and(pc_ >= lim1, pc_ < lim2)]) / gridSize2)
        pc_[pc_ < lim2] = np.round(pc_[pc_<lim2] / gridSize2)
