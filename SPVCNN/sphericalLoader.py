...
class SphericalSemanticKITTI(dict):
    def __init__(self, root, voxel_size, num_points, **kwargs):
        ...
        if submit_to_server:
            super(SphericalSemanticKITTI, self).__init__({
                'train':
                SmodifyPointCloudSemanticKITTIInternal(root,
                                      voxel_size,
                                      num_points,
                                      sample_stride=1,
                                      split='train',
...
class SmodifyPointCloudSemanticKITTIInternal:
    def __init__(self,
                 root,
                 voxel_size,
                 num_points,
    ...
    def __getitem__(self, index):
        ...
        fi = np.arctan2(block[:, 1], block[:, 0])  # poziom
